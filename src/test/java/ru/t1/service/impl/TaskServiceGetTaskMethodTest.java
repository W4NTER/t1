...
public class TaskServiceGetTaskMethodTest {
    @Mock
    ...
    @Test
    @DisplayName("тест получения задачи по идентификатору")
    void testThatGetTaskReturnedSucceed() {
        Long taskId = 1L;
        ...
        assertNotNull(res);
        assertEquals(taskEntity.getTitle(), res.title());
        assertEquals(taskEntity.getDescription(), res.description());
        assertEquals(taskEntity.getStatus(), res.status());
    }

    @Test
    @DisplayName("тест получения задачи по идентификатору, выброс ошибки EntityNotFound")
    void testThatGetTaskThrowsException() {
        Long taskId = 1L;
